...
def test_wrong_report():
    args = Args(files=['tests/test_data/data1.csv', 'tests/test_data/data2.csv', 'tests/test_data/data3.csv',], report='nonexistent_report')
    with pytest.raises(ValueError) as exception:
        validate_input_data(args)

def test_all_files_valid_real():
    args = Args(files=['tests/test_data/data1.csv', 'tests/test_data/data2.csv', 'tests/test_data/data3.csv',],)
    validate_input_data(args)
