...
def parser():
    return CSVParser('tests/test_data/data1.csv', 'tests/test_data/data2.csv', 'tests/test_data/data3.csv')

def test_calculate_field_functions(parser):
    report = parser.reports[0]
    data_dict = {key: int(val) if val.isdigit() else val for key, val in zip(parser.headers, report)}
