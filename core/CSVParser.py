...
class CSVParser:
    def __init__(self, *filepaths: str):
        ...
        self.columns_longest_sizes = [
            len(COLUMNS_ORIGINALS[i]) for i in range(COLUMNS_COUNT)
        ]
    ...
    def _map_headers(self, header_row):
        ...
        missing_fields = [
            COLUMNS_ORIGINALS[i]
            for i in range(COLUMNS_COUNT)
            if not column_occurrences[i]
        ]
        if missing_fields:
            raise ValueError(f"Missing fields: {", ".join(missing_fields)}")

        duplicate_fields = [
            tuple(column for column in COLUMNS_AND_ALIASES[i] if column in header_row)
            for i in range(COLUMNS_COUNT)
            if column_occurrences[i] > 1
        ]
        if duplicate_fields:
        ...
        with open(file, "r") as f:
            ...
            for line in f:
                ...
                for i in range(len(values)):
                    ...
                    self.columns_longest_sizes[COLUMN_INDEXES[headers[i]]] = max(
                        self.columns_longest_sizes[COLUMN_INDEXES[headers[i]]],
                        len(values[i]),
                    )
    ...
    def _print_headers(self, fields_to_print, calculated_fields_functions):
        print(
            "".ljust(
                self.columns_longest_sizes[COLUMN_INDEXES["department"]] + self.shift
            ),
            end="",
        )
        for i in range(len(self.headers)):
            if fields_to_print[i]:
                print(
                    self.headers[i].ljust(self.columns_longest_sizes[i] + self.shift),
                    end="",
                )
    ...
    def _print_row(self, report, fields_to_print):
        print(
            "".ljust(
                self.columns_longest_sizes[COLUMN_INDEXES["department"]]
                + self.shift
                - 1,
                "-",
            ),
        ...
        for i in range(len(report)):
            if fields_to_print[i]:
                print(
                    str(report[i]).ljust(self.columns_longest_sizes[i] + self.shift),
                    end="",
                )
    ...
    def _print_calculated_data(self, report, calculated_fields_functions):
        report_dct = {}
        for i in range(len(report)):
            report_dct[COLUMNS_ORIGINALS[i]] = (
                int(report[i]) if report[i].isdigit() else report[i]
            )
        for func in self.calc_field_funtions:
            if func.name in calculated_fields_functions:
    ...
    def output(self, *args, calculated_fields_functions=None):
        if args:
            fields_to_print = [
                COLUMNS_ORIGINALS[i] in args and COLUMNS_ORIGINALS[i] != "department"
                for i in range(COLUMNS_COUNT)
            ]
        else:
            fields_to_print = [
                COLUMNS_ORIGINALS[i] != "department" for i in range(COLUMNS_COUNT)
            ]
        i = 0
    ...
    def generate_groups(self, group_by="department"):
        ...
        for report in self.reports:
            ...
            for i in range(len(report)):
                ...
                groups[group][COLUMNS_ORIGINALS[i]].append(
                    int(report[i]) if report[i].isdigit() else report[i]
                )
        return groups

    def output_group(
        self, group_by="department", calculated_group_fields_functions=None
    ):
        print(
            group_by.ljust(
                self.columns_longest_sizes[COLUMN_INDEXES[group_by]] + self.shift
            ),
            end="",
        )
        ...
        for group_key, group_data in groups.items():
            ...
            for func in self.calc_group_filed_frunctions:
                if func.name in calculated_group_fields_functions:
                    print(
                        str(func(group_data)).ljust(func.max_size + self.shift), end=""
                    )
            print()
